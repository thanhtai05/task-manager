...
const connectDatabase = async () => {
  ...
  try {
    // Prefer connecting to a real MongoDB when URI is provided
    if (config.MONGO_URI) {
    ...
    // In development or when explicitly enabled, start in-memory Mongo without logging an error
    if (useMemory) {
    ...
    // No URI and not allowed to use memory => hard fail
    ...
    // If connection fails and memory is allowed, start in-memory Mongo
    if (useMemory) {
      console.log("Starting in-memory MongoDB Replica Set for development...");
      const memoryPort = Number(process.env.MONGO_MEMORY_PORT || 27017);
      const dbPathEnv = process.env.MONGO_MEMORY_DBPATH;
